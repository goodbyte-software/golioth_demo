...
static void selected_led_state_handler(struct golioth_client* client, const struct golioth_response* response, const char* path, void* arg)
{
...
void update_lightdb_state_work_handler(struct k_work* work)
{
    ...
    int err = golioth_lightdb_set_string_async(client, "selected_led", _led_sel ? "LED1" : "LED0", 4, selected_led_state_handler, NULL);
    if (err) {
...
int main(void)
{
    ...
    while (true) {
        ...
        bool ok = zcbor_map_start_encode(zcbor_e_state, 1) && zcbor_tstr_put_lit(zcbor_e_state, "temp") &&
            zcbor_float64_put(zcbor_e_state, sensor_value_to_double(&temperature)) && zcbor_map_end_encode(zcbor_e_state, 1);

        if (ok) {
            size_t payload_length = zcbor_e_state->payload - buf;

            LOG_INF("Streaming Temperature to Golioth: %i.%06u", temperature.val1, abs(temperature.val2));

            err = golioth_stream_set_async(client, "", GOLIOTH_CONTENT_TYPE_CBOR, buf, payload_length, temperature_push_handler, NULL);
            if (err) {
